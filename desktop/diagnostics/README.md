...
- `markdown/`：检查原始 Markdown 的写法，在内存副本上做有限修复。
- `layout/`：为今后的 HTML/PDF 几何检查预留入口，如溢出、裁切、分页。居中指令未被保留属于渲染/布局错误，不应通过改写 Markdown 掩盖。现有布局逻辑与回归测试继续保留在原位置。

流程：原文件 → `checkMarkdown` → 内存副本 → Markdown/HTML 渲染 → PDF → 预览/导出。

本目录不访问文件系统。`document.cjs` 只读原文件，检测结果通过 `sourceDiagnostics` 进入单文件/批量预览；修复不会写回原文件，也不会新建一份“修复后的 Markdown”。导出仍使用预览对应的 PDF 字节。
...
1. 在 `markdown/` 添加纯函数规则，导出稳定的 `id` 与 `check({ lines, paragraphs })` 生成器，在 `index.cjs` 注册。
