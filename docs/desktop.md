...
Version 0.6.1: preview issues also appear above the PDF. Choose **View details** to expand the document notices, including image references and specific failure reasons. Closing the notification keeps the details available below the preview. Successfully repaired syntax remains an informational notice. Preview and export failures use an error card. Saving a PDF does not dismiss warnings about missing content, so saving successfully is distinct from rendering every resource successfully.
...
Inkleaf offers Modern (default), Classic and Minimal in Settings → Layout → Theme. Modern includes bundled Inter and JetBrains Mono Latin fonts, a six-level heading hierarchy, code cards with language labels and optional line numbers, and dedicated quotation/callout styling. Existing saved theme choices are retained. The reusable [quality template and grayscale checks](../tests/quality/README.md) cover all three themes; see [release notes](releases/0.6.1.md).

Version 0.6.1: Modern also bundles Noto Sans SC for Chinese document text, headings, tables, code annotations and signatures. It retains the sans-serif appearance; the experimental serif font is not included. Only required Unicode subsets are embedded, with no font downloads during conversion. The 101 bundled WOFF2 subsets total about 4.3 MiB. Inter and JetBrains Mono remain first for Latin and code; Classic and Minimal keep their existing font choices. Browser-generated page headers/footers and characters outside the bundled repertoire still use system fonts. See [font source and license](../resources/fonts/noto-sans-sc/README.md).
...
When opened, search is horizontally centered on the display containing the pointer, with its top at about 16% of that display's usable height. Results expand downward. Automatic width is 30% of the usable display width plus 120 logical pixels, bounded to 560–640 logical pixels. Drag either side of the search window to set a preferred width (minimum 420 pixels); this is saved locally and restored after restarting. Dragging the top or bottom changes the preferred base height symmetrically around the center (minimum 62 pixels); search results add their content height. Width and height preferences are saved independently. Opening a preview or settings does not change the search-width preference. On a smaller monitor, the visible width is limited to fit without replacing the saved preference. Monitor changes and display scaling are handled in logical pixels. Native edge-drag preference handling targets Windows/macOS; Linux packaging remains unvalidated.
...
HTTPS raster images and SVG badges are downloaded before printing, then embedded as data images. Local raster/SVG images are supported too. Within a Git project, relative paths may reference shared assets anywhere inside the nearest project root (including worktrees); standalone documents stay limited to their own folder tree. Resolved paths are checked after following filesystem links. Small SVG badges stay inline, with their native size, text, gradients and embedded logos. Their alignment follows the original paragraph or container: `align` attributes and inline `text-align` styles are preserved. Scripts and external SVG resources are removed. Fetching uses public HTTPS addresses, no cookies or document uploads, bounded redirects, ten-second request deadlines and a 15 MB limit per image. Each document resolves up to 100 unique images with four concurrent loads. Remote images are cached in memory for five minutes (up to 128 images / 32 MB). A new remote image requires a connection; exported PDFs contain their own image bytes and work offline. Missing images show alt text and an explicit warning rather than silently disappearing.
...
npm run dist:win   # release/Inkleaf-Setup-0.6.1.exe
```
...
- Markdown files up to 10 MB. Relative SVG/PNG/JPEG/GIF/WebP images within the nearest Git project, or the Markdown file's directory tree when no Git project is found, and public HTTPS images are embedded, up to 15 MB each. Unavailable or unsupported images and local images outside that boundary produce a preview warning. KaTeX and its fonts are bundled for offline use.
