...
Themes: Modern (default), Classic and Minimal. New profiles, missing/invalid theme preferences and Reset this category use Modern; resetting Layout remains a draft until saved. Retired Folio and Dark preferences fall back to Modern without changing other settings. Existing saved Classic and Minimal choices are retained. Paper: A4 or Letter, portrait or landscape. Margins: compact, standard or wide. Body text: 8–18 pt. Line spacing: 1.4, 1.6 or 1.8. Signature and copyright can be enabled/disabled independently of the stored name; existing `author` / `authorEnabled` preferences remain compatible. Page numbers have their own switch.
