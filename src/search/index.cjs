...
function compareValues(rankA, a, rankB, b) {
...
class SearchIndex {
  ...
  search(query) {
    ...
    const heap = [];
    let matches = 0;
    // 只保留最好的 80 项，堆顶是其中最差的一项。
    // 这样无需收集、排序几十万个匹配结果；文件和文件夹共用这 80 个位置。
    ...
    for (const record of this.folderEntries.values()) {
      ...
      matches++;
      let rank = 3;
      if (record.lower === text) rank = 0;
      else if (record.lower.startsWith(text)) rank = 1;
      else if (nameOnly) rank = 2;
      retain(record, rank);
    }
    for (const record of this.records.values()) {
      let nameOnly = true,
      ...
      matches++;
      let rank = 3;
      if (record.stem === text || record.lower === text) rank = 0;
      else if (record.stem.startsWith(text)) rank = 1;
      else if (nameOnly) rank = 2;
      retain(record, rank);
    }
    const result = {
      files: heap.sort(compare).map((item) => {
        ...
        if (entry.kind === "folder") entry.count = record.count;
        return entry;
      }),
      matches,
    };
